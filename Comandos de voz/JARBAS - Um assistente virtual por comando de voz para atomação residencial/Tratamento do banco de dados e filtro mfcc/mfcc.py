...
# processamento e geração das componentes cepstrais para o dataset de treinamento da rna
...
# Escrevendo os dados para o arquivo CSV de treino da RNA
...
# processamento e geração das componentes cepstrais para o dataset de validação da rna
for comando in range(quantidadeComandos):
    if (comando == 0):
        comando_aux = f'{pasta}/10-Jarbas'
    if (comando == 1):
    ...
    # número de amostras de áudio contida em cada pasta de comando (Ligue, Desligue, Jarbas...)
    for speaker in range(inicioTeste, finalTeste):
        audio = f'{comando_aux}/{speaker}.wav'
        [fs, xi] = wav.read(audio)
...
# Escrevendo os dados para o arquivo CSV de Validação da RNA
arquivo = f"{diretorio_atual}/testecsvteste.csv"
with open(arquivo, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=';')
    writer.writerows(tabela)
    print("testecsvteste gravado")
    tabela.clear()
